...
describe('refresh', () => {
  ...
  it('should generate stuff', async () => {
    ...
    expect(tree.read('apps/my-app/src/main.ts', 'utf-8'))
      ...
      .action(async (a,b,opts) => { await appleCommand({ a,b,opts }); });
      prog
      .command('banana <e> <f>')
      ...
      .action(async (e,f,opts) => { await bananaCommand({ e,f,opts }); });
...
describe('refresh no commands', () => {
  ...
  beforeEach(async () => {
    ...
    await applicationGenerator(tree, {
      name: projectName,
      directory: joinPathFragments('apps', projectName),
      skipFormat: true,
    });
    const workspace = readNxJson(tree);
    if (!workspace) throw new Error('no nx');
  ...
  it('should generate stuff', async () => {
    ...
    expect(tree.children('apps/my-app/src/app')).toContain('commands');
